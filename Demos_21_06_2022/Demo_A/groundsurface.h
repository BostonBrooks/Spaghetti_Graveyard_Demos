...
//vertex_shader
    ...
    char fragShader[] = "\
        ...
        void main()\
        {\
            vec4 pixel = texture2D(Base_Texture, gl_TexCoord[0].xy);\
        \
            vec4 Light = texture2D(Hill_Shading_Texture, gl_TexCoord[0].xy);\
...
int Draw_Ground_Square(int Square_i, int Square_j){
    ...
    Screen_Coords top_corner;
    top_corner = tile_to_screen_centre_viewpoint(TC);
    sfVector2f sf_top_corner;
    sf_top_corner.x = top_corner.x;
    sf_top_corner.y = top_corner.y;
    
    sfVertexArray* VA = Ground_Surface->Vertex_Array;
    
    sfShader_setVec2Uniform (GroundShader, "offset", sf_top_corner);
