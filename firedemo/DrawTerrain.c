...
#define ELEVATIONSCALE       *1
#define ELEVATIONRANGE       256
...
IJK_Point XYtoIJmaxrelative ( XY_Point a ) {
    IJK_Point b;
    a.y += ELEVATIONRANGE ;
    b.k = ELEVATIONRANGE ;
...
int GenDrawables (int SquareM, int SquareN) {
    ...
    //for (TileM = 0; TileM < SUBDIVISIONS; TileM += 8) {
    //for (TileN = 0; TileN < SUBDIVISIONS; TileN += 8) {
    if (rand() % 2 == 0) {
        IJK_Point location;
        location.i = MAPSQUARESIZE * SquareM + MAPTILESIZE * TileM + MAPTILESIZE/4 + rand() % MAPTILESIZE/2;
        location.j = MAPSQUARESIZE * SquareN + MAPTILESIZE * TileN + MAPTILESIZE/4 + rand() % MAPTILESIZE/2;
...
int GenSquares(void) {
    ...
    HeightMap = sfImage_createFromFile("HeightMapCheck.bmp");
    assert (HeightMap != NULL);
    HeightMapPtr = sfImage_getPixelsPtr(HeightMap);
    HeightMapSize = sfImage_getSize(HeightMap);

    GroundSurface = sfTexture_createFromFile("grass3.bmp", NULL);
...
#define CLUDGE 256
    ...
    a.x = 0;
    a.y = SCREEN_HEIGHT + ELEVATIONRANGE + CLUDGE - ViewPoint.k;
...
int CreateSprites() {
    ...
    TreeTexture[0] = sfTexture_createFromFile("fire.bmp", NULL);
...
int PlotVisibleDrawables() {
    ...
    Drawable* CurrentDrawable_ = VisibleDrawable_Head;
...
int sign (XY_Point p1,XY_Point p2,XY_Point p3) {

    return (p1.x - p3.x) * (p2.y - p3.y) - (p2.x - p3.x) * (p1.y - p3.y);
}

int IsXYinIJKTrig(XY_Point a, IJK_Point b1, IJK_Point b2, IJK_Point b3) {

    XY_Point c1, c2, c3;

    c1 = IJKtoXYrelative(b1);
    c2 = IJKtoXYrelative(b2);
    c3 = IJKtoXYrelative(b3);

    int d1,d2,d3;

    d1 = sign (a, c1, c2) < 0? 1 : 0;
    d2 = sign (a, c2, c3) < 0? 1 : 0;
    d3 = sign (a, c3, c1) < 0? 1 : 0;


    return ((d1 == d2) && (d2 == d3));
}

IJK_Point mapXYtoIJKTrig(XY_Point a, IJK_Point b1, IJK_Point b2, IJK_Point b3) {
    ...
    //        | b1.i  c1.y  1 |           | c1.x  b1.i  1 |             | c1.x  c1.y  b1.i |      | c1.x  c1.y  1 |
    //P.i = ( | b2.i  c2.y  1 | * a.x   + | c2.x  b2.i  1 | * a.y     + | c2.x  c2.y  b2.i |  ) / | c2.x  c2.y  1 |
    //        | b3.i  c3.y  1 |           | c3.x  b3.i  1 |             | c3.x  c3.y  b3.i |      | c3.x  c3.y  1 |

//Det 1
    // | b1.i  c1.y  1 | = -c1.y b2.i + c1.y b3.i + c2.y b1.i - c2.y b3.i - c3.y b1.i + c3.y b2.i
    // | b2.i  c2.y  1 |
    // | b3.i  c3.y  1 |

//Det 2
    // | c1.x  b1.i  1 | = -b1.i c2.x + b1.i c3.x + b2.i c1.x - b2.i c3.x - b3.i c1.x + b3.i c2.x
    // | c2.x  b2.i  1 |
    // | c3.x  b3.i  1 |

//Det 3
    // | c1.x  c1.y  b1.i | = c1.x c2.y b3.i - c1.x c3.y b2.i - c2.x c1.y b3.i + c2.x c3.y b1.i + c3.x c1.y b2.i - c3.x c2.y b1.i
    // | c2.x  c2.y  b2.i |
    // | c3.x  c3.y  b3.i |

//Det 4
    // | c1.x  c1.y  1 | = -c1.y c2.x + c1.y c3.x + c2.y c1.x - c2.y c3.x - c3.y c1.x + c3.y c2.x
    // | c2.x  c2.y  1 |
    // | c3.x  c3.y  1 |

    int Det1, Det2, Det3, Det4;

    Det1 = -c1.y * b2.i + c1.y * b3.i + c2.y * b1.i - c2.y * b3.i - c3.y * b1.i + c3.y * b2.i;
    Det2 = -b1.i * c2.x + b1.i * c3.x + b2.i * c1.x - b2.i * c3.x - b3.i * c1.x + b3.i * c2.x;
    Det3 = c1.x * c2.y * b3.i - c1.x * c3.y * b2.i - c2.x * c1.y * b3.i + c2.x * c3.y * b1.i + c3.x * c1.y * b2.i - c3.x * c2.y * b1.i;
    Det4 = -c1.y * c2.x + c1.y * c3.x + c2.y * c1.x - c2.y * c3.x - c3.y * c1.x + c3.y * c2.x;

    assert(Det4 != 0);

    P.i = (Det1 * a.x + Det2 * a.y + Det3)/Det4;

    Det1 = -c1.y * b2.j + c1.y * b3.j + c2.y * b1.j - c2.y * b3.j - c3.y * b1.j + c3.y * b2.j;
    Det2 = -b1.j * c2.x + b1.j * c3.x + b2.j * c1.x - b2.j * c3.x - b3.j * c1.x + b3.j * c2.x;
    Det3 = c1.x * c2.y * b3.j - c1.x * c3.y * b2.j - c2.x * c1.y * b3.j + c2.x * c3.y * b1.j + c3.x * c1.y * b2.j - c3.x * c2.y * b1.j;
    Det4 = -c1.y * c2.x + c1.y * c3.x + c2.y * c1.x - c2.y * c3.x - c3.y * c1.x + c3.y * c2.x;

    assert(Det4 != 0);

    P.j = (Det1 * a.x + Det2 * a.y + Det3)/Det4;

    Det1 = -c1.y * b2.k + c1.y * b3.k + c2.y * b1.k - c2.y * b3.k - c3.y * b1.k + c3.y * b2.k;
    Det2 = -b1.k * c2.x + b1.k * c3.x + b2.k * c1.x - b2.k * c3.x - b3.k * c1.x + b3.k * c2.x;
    Det3 = c1.x * c2.y * b3.k - c1.x * c3.y * b2.k - c2.x * c1.y * b3.k + c2.x * c3.y * b1.k + c3.x * c1.y * b2.k - c3.x * c2.y * b1.k;
    Det4 = -c1.y * c2.x + c1.y * c3.x + c2.y * c1.x - c2.y * c3.x - c3.y * c1.x + c3.y * c2.x;

    assert(Det4 != 0);

    P.k = (Det1 * a.x + Det2 * a.y + Det3)/Det4;
...
sfCircleShape* CircleShape2;

//return 1 if mousepos is in the lower half of the tile
//return 2 if mousepos is in the upper half of the tile,
//else return 0;
int CheckTile(int M, int N, XY_Point mousepos) {

    int TSN, TSM, TILEN, TILEM;

    TSM = M / SUBDIVISIONS;
    TSN = N / SUBDIVISIONS;
    TILEM = M % SUBDIVISIONS;
    TILEN = N % SUBDIVISIONS;

    IJK_Point LeftPt, TopPt, RightPt, BottomPt;

    LeftPt.i = M * TEXTURETILESIZE;
    LeftPt.j = N * TEXTURETILESIZE;
    LeftPt.k = TerrainSquares[TSM][TSN].ElevationData[TILEM+1][TILEN+1];

    XY_Float leftpos = IJKtoXYfloat (LeftPt);
    ...
    //sfCircleShape_setPosition(CircleShape2, loc);
    //sfRenderWindow_drawCircleShape(window, CircleShape2, NULL);

    TopPt.i = M * TEXTURETILESIZE;
    TopPt.j = (N+1) * TEXTURETILESIZE;
    TopPt.k = TerrainSquares[TSM][TSN].ElevationData[TILEM+1][TILEN+1+1];


    XY_Float toppos = IJKtoXYfloat (TopPt);
    loc.x = toppos.x;
    loc.y = toppos.y;
    ...
    RightPt.i = (M+1) * TEXTURETILESIZE;
    RightPt.j = (N+1) * TEXTURETILESIZE;
    RightPt.k = TerrainSquares[TSM][TSN].ElevationData[TILEM+1+1][TILEN+1+1];

    XY_Float rightpos = IJKtoXYfloat (RightPt);
    loc.x = rightpos.x;
    loc.y = rightpos.y;
    ...
    BottomPt.i = (M+1) * TEXTURETILESIZE;
    BottomPt.j = N * TEXTURETILESIZE;
    BottomPt.k = TerrainSquares[TSM][TSN].ElevationData[TILEM+1+1][TILEN+1];

    XY_Float bottompos = IJKtoXYfloat (BottomPt);
    loc.x = bottompos.x;
    loc.y = bottompos.y;
    ...
    if (IsXYinIJKTrig(mousepos, LeftPt, BottomPt, RightPt)) {

        printf ("Bottom\n");
        return 1;

    }
    if (IsXYinIJKTrig(mousepos, LeftPt, TopPt, RightPt)) {
...
int LeftOrRight(int i, int j) {
    if (i+j < MAPTILESIZE) return 1;
    else return 0;
}

IJK_Point XYtoIJKrelative (XY_Point xypoint) {


        if (xypoint.x>0 && xypoint.x<SCREEN_WIDTH && xypoint.y>0 && xypoint.y<SCREEN_HEIGHT)
        {
            IJK_Point mousepoint = XYtoIJmaxrelative (xypoint);

            int MMAX = mousepoint.i / MAPTILESIZE;
            int NMAX = mousepoint.j / MAPTILESIZE;

            int SubI = mousepoint.i % MAPTILESIZE;
            int SubJ = mousepoint.j % MAPTILESIZE;

            mousepoint = XYtoIJ0relative (xypoint);

            int MMIN = mousepoint.i / MAPTILESIZE;
            int NMIN = mousepoint.j / MAPTILESIZE;
            ...
            int TopOrBottom = 0;
            if (LeftOrRight(SubI, SubJ)) {
                while (NX < NMIN+1 || MX > MMIN-1) {

                    if (TopOrBottom = CheckTile(MX, NX,xypoint)) break;
                    MX -= 1;
                    if (TopOrBottom = CheckTile(MX, NX,xypoint)) break;
                    NX += 1;
                }
            ...
            LeftPt.i = MX * TEXTURETILESIZE;
            LeftPt.j = NX * TEXTURETILESIZE;
            LeftPt.k = TerrainSquares[MX/SUBDIVISIONS][NX/SUBDIVISIONS].ElevationData[MX%SUBDIVISIONS+1][NX%SUBDIVISIONS+1];

            TopPt.i = MX * TEXTURETILESIZE;
            TopPt.j = (NX+1) * TEXTURETILESIZE;
            TopPt.k = TerrainSquares[MX/SUBDIVISIONS][NX/SUBDIVISIONS].ElevationData[MX%SUBDIVISIONS+1][NX%SUBDIVISIONS+1+1];

            RightPt.i = (MX+1) * TEXTURETILESIZE;
            RightPt.j = (NX+1) * TEXTURETILESIZE;
            RightPt.k = TerrainSquares[MX/SUBDIVISIONS][NX/SUBDIVISIONS].ElevationData[MX%SUBDIVISIONS+1+1][NX%SUBDIVISIONS+1+1];

            BottomPt.i = (MX+1) * TEXTURETILESIZE;
            BottomPt.j = NX * TEXTURETILESIZE;
            BottomPt.k = TerrainSquares[MX/SUBDIVISIONS][NX/SUBDIVISIONS].ElevationData[MX%SUBDIVISIONS+1+1][NX%SUBDIVISIONS+1];
            ...
            if (TopOrBottom == 1 ) { //bottom
                foo = mapXYtoIJKTrig(xypoint, LeftPt, BottomPt, RightPt);
            }

            if (TopOrBottom == 2) { //top

                foo = mapXYtoIJKTrig(xypoint, LeftPt, TopPt, RightPt);
            }

            printf("foo = %d, %d\n", foo.i + foo.j, foo.k);

            printf("mousepoint = %d\n", mousepoint.i + mousepoint.j);

            sfVector2f loc;
            XY_Float bottompos = IJKtoXYfloat (foo);
            loc.x = bottompos.x;
            loc.y = bottompos.y;
            sfCircleShape_setPosition(CircleShape2, loc);
            sfRenderWindow_drawCircleShape(window, CircleShape2, NULL);
...
int GetMouse (void) {
    if (sfMouse_isButtonPressed (sfMouseRight)) {
        ...
        GoalPt = XYtoIJKrelative  (xymousestate);
...
int Select(sfEvent click) {
    ...
    XY_Point MousePos;
    MousePos.x = click.mouseButton.x;
    MousePos.y = click.mouseButton.y;

    IJK_Point MapPos = XYtoIJKrelative (MousePos);
    CreateDrawable(MapPos);
...
int main(void) {
    ...
    while(1) {
        ...
        IJK_Point ViewPointDelta;

        ViewPointDelta.i = GoalPt.i - ViewPoint.i;
        ViewPointDelta.j = GoalPt.j - ViewPoint.j;
        int DeltaSquared = ViewPointDelta.i*ViewPointDelta.i + ViewPointDelta.i*ViewPointDelta.i;

        #define SPEED  8
        if (DeltaSquared< SPEED*SPEED){
            ViewPoint = GoalPt;
        } else {
            ViewPoint.i+= ViewPointDelta.i * SPEED / sqrt(DeltaSquared);
            ViewPoint.j+= ViewPointDelta.j * SPEED / sqrt(DeltaSquared);
        }
